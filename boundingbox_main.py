...
class cropBox(object):
    def __call__(self, img):
        print(img['start'], img["vol"]) # --> das is leer well probaply cause i dont have all the boxes :/ ohne shuffle is problem auch noch da
        image["vol"] = img["vol"].SpatialCrop(roi_start=img['start'], roi_end=img['end'])
        image["seg"] = img["seg"].SpatialCrop(roi_start=img['start'], roi_end=img['end'])
        return image
...
validation_transforms = Compose(
    [
        ...
        #SpatialCropd(keys=["vol", "seg"], roi_start=all_files['start'], roi_end=all_files['end']),
        cropBox(),
...
train_transforms = Compose(
    [
        ...
        Orientationd(keys=["vol", "seg"], axcodes="PLS"),
        ScaleIntensityRanged(keys=["vol"], a_min=a_min, a_max=a_max, b_min=0.0, b_max=1.0, clip=True),
        #SpatialCropd(keys=["vol", "seg", "start", "end"], roi_start=box_start, roi_end=box_end),
        cropBox(),
        Resized(keys=["vol", "seg"], spatial_size=spatial_size),
        ToTensord(keys=["vol", "seg"]),
    ]
